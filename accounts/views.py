...
def dashboard(request):
    top_reminders = Reminder.objects.filter(importance='very-important').order_by('date', 'time')
    username = request.user.username  # Fetch the username of the logged-in user
    return render(request, 'dashboard.html', {'top_reminders': top_reminders, 'Username': username})
...
def videocall(request):
    return render(request, 'accounts/videocall.html', {'name': request.user.first_name + " " + request.user.last_name})
...
def join_room(request):
    ...
    return render(request, 'accounts/joinroom.html')
